...
- **⚠️ KONSTITUSI DIPERBARUI (2026-06-19): `aplikasi.md` v1.3 → v1.4** — Ditambah **BAGIAN 12B (Adendum Roadmap Engagement/Karier/Wellness)**: 10 fitur (F1–F10) + 5 ADR-baru placeholder, dipetakan ke fase+guardrail+status buildable/gated. **Fase 0–8, 12A, ADR-001..008 TIDAK berubah.** Sumber detail: `docs/refs/konsep-engagement-karier.md`. ⚠️ **F7 (dashboard kepsek) BENTROK guardrail 13.6 → tertahan sampai owner rekonsiliasi 13.6.** Item uang/showcase-anak/kesehatan tetap gated (ADR+hukum). Tidak mengubah pekerjaan Fase 2 berjalan.
