...
- **Cek konformitas sebelum commit (WAJIB):** panggil `magnoo-architect` **MODE C** atas file/diff yang kamu tulis → pastikan patuh ADR, model data/API sesuai spec, aturan bisnis BAGIAN 10, tanpa scope-creep. Bila ada LANGGAR → perbaiki dulu, jangan commit.
- **Audit keamanan sebelum commit (WAJIB untuk potongan backend/auth/data/endpoint):** panggil `magnoo-security` (bila tipe belum dikenal di sesi → `general-purpose` + suruh baca `/root/.claude/agents/magnoo-security.md`) atas file/diff → IDOR, RBAC scope, kebocoran PII/NIS anak, secret, guardrail BAGIAN 13/14, OWASP. Verdikt **DITAHAN** (ada CRITICAL/HIGH) → perbaiki dulu, JANGAN commit. Ini garis mati (data anak).
- **Tes (WAJIB — "selesai = teruji"):** panggil `magnoo-tester` untuk menulis/melengkapi tes aturan bisnis BAGIAN 10 + keamanan (IDOR/RBAC) + edge-case, lalu jalankan. Semua HARUS hijau; ada celah/merah → tambal dulu. Untuk penutup fase: E2E DoD (BAGIAN 15).
- **Ringkas & ringankan (sebelum commit):** panggil `magnoo-simplifier` atas diff → buang duplikasi/dead-code/kerumitan, jaga bobot (portal <200KB, APK <40MB). Terapkan usulan "aman & jelas" (yang ditutup tes); usulan berisiko → tandai utang. Boleh juga skill bawaan `/simplify`.
