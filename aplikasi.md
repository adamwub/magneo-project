...
>
> **FILOSOFI DESAIN INTI (kompas semua fitur di 12B):** (1) **partisipasi-dulu, kreasi-opsional**
> — keseruan tak boleh bergantung pada "semua jadi kreator"; poin dari partisipasi (hadir/kuis/bantu/
> belajar), kreasi = bonus. (2) **ambang kreasi serendah teks/suara + AI-assist** (keahlian dihargai,
> bukan bakat ngonten). (3) **admin di belakang layar** (app meringankan dulu, baru fun). (4) **metrik
> sehat** (progres, bukan like/popularitas) + anti-kanvas-kosong (seed + misi mingguan). Detail di konsep.
>
> **F7 & guardrail 13.6 — SUDAH DISELESAIKAN (tanpa melemahkan 13.6):** F7 dibalik jadi **berpusat
> siswa/kelas** (kesehatan kelas, EWS, agregat — unit kelas/siswa, BUKAN skor per guru); analitik guru =
> cermin pribadi guru (opt-in), bukan ranking yang diakses kepsek. 13.6 tetap utuh. (ADR-baru-E.)
...
| **F7 Dashboard Kepsek (BERPUSAT SISWA/KELAS)** | Kesehatan kelas & kesejahteraan siswa: tren kehadiran per kelas, EWS kelas berisiko, agregat sekolah, "kelas ini butuh dukungan". Unit = kelas/siswa, BUKAN skor guru. Analitik guru = cermin pribadi guru (opt-in), bukan diakses kepsek. | Fase 8 (Analitik) + AI (Fase 4). | 13.6 **tetap utuh** (tak ada ranking/pemakaian per guru utk kepsek); ADR-005 (ID/agregat, tanpa nama siswa); ADR-baru-E. | gated-ADR (E) — desain 13.6-safe |
| **F11 Kanal Konseling BK** | Siswa konsultasi/curhat ke guru BK, privat dari ortu (ortu beri izin, tak memantau). | Ekstensi `comms`/BK. | Privat-dari-ortu ≠ gelap: peran BK akuntabel, terekam akses-terkunci (break-glass), eskalasi wajib; bukan langgar 13.5 tapi pengecualian terkendali; data konseling sensitif PDP. | **gated-ADR+hukum (F)** |
| **F12 Alat TU (AI + blast)** | TU: AI buat draf surat/rekap, blast pengumuman/surat massal, ekspor/cetak sekali-tap. | `school` admin + AI (Fase 4/5). | blast tunduk scope×role 12A.4 + batas frekuensi + audit; AI via ADR-007; tanpa PII anak keluar. "TU" belum jadi role → SCHOOL_ADMIN/sub-role. | buildable-now (saat fasenya; sebagian dekat fitur admin existing) |
| **F13 Ruang Kreasi (konten guru/siswa)** | Guru unggah materi/Open Class; siswa unggah karya/eksperimen → poin + Skill Passport. Wujud Filosofi 1B. | Fase 5 (engagement). | **UGC anak = paling berisiko:** verifikasi-dulu-baru-tayang (guru approve), audiens dalam-sekolah, izin ortu utk citra anak (ADR-005), metrik sehat, AI-assist. | **gated-ADR+hukum (B/C, moderasi+consent)** |
| **F14 Sambungan Startup Center (ide→mentor→investor)** | Ide/karya terbaik naik ke Startup Center (Fase 8, sudah ada): dimentori Magneo crew+alumni, demo day, lalu disambung investor. Puncak tangga: belajar→karya→portofolio→inovasi→didanai. | Mentoring/idea = Fase 8 (sebagian ada); sambungan investor = fase gated. | Siswa=anak → koneksi investor gated (izin ortu, dimediasi mentor/sekolah, investor tak kontak anak langsung, kejelasan IP/kepemilikan ide); alumni=bersih; sekuritas/IP/pajak = hukum. | Mentoring (Magneo crew): buildable saat Fase 8. **Sambungan investor: gated-ADR+hukum (A/B)** |
...
- **ADR-baru-E — Dashboard kepsek berpusat siswa/kelas (F7):** unit analisis = kelas/siswa (kesehatan kelas, EWS, agregat), BUKAN skor/ranking guru → **13.6 tetap utuh**; analitik guru = cermin pribadi guru (opt-in), bukan diakses kepsek; AI = decision-support explainable (ADR-005).
- **ADR-baru-F — Kanal konseling BK (F11):** privat dari ortu (consent ortu di depan) tapi **terekam akses-terkunci (break-glass) + peran BK akuntabel + eskalasi wajib**; bukan kanal gelap (tak langgar 13.5); data konseling = sensitif PDP. Pengecualian terkendali, gerbang ADR+hukum.
